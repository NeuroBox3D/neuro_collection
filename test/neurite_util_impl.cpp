...
//		}
		...
		(
			...
			for (size_t i = 0; i < numNeurites; i++) {
				...
				for (size_t j = 0; j < numVerts; j++) {
					UG_DLOGN(NC_TNP, 0, "Position of projected vertex: " << aaPos[projectedVertices[j]]);
	...
	/*		UG_COND_THROW(fabs(refVec.x()) < SMALL || fabs(refVec.y()) < SMALL || fabs(refVec.z()) < SMALL,
			...
			UG_DLOGN(NC_TNP, 0, "Optimal direction index " << optimal << " with vector coordinates " << refVec << "found for polygon");
			refVec = dirs[optimal];
			for (size_t j = 0; j < numVerts; j++) {
				angleMapFrom.clear(); angleMapTo.clear();
